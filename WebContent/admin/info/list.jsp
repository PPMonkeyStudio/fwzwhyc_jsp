...
<body>
    ...
    <div class="tools">    
    	<ul class="toolbar">
        <li class="click"><a href="${path }admin/info/add.jsp"><span><img src="images/t01.png" /></span>添加信息</a></li>
        </ul>
       </div>
       <form action="${path }admin/findInfosByPage" id="infoListForm" name="ff" method="post"> 
	       <ul class="seachform">
		    <li><label>标题关键词</label><input name="keywords" value="${keywords }" type="text" style="width:400px;" class="scinput" /></li>
				...
				<c:if test="${!empty pb.list}">	
					<s:iterator value="pb.list" id="info">	
						<tr>
							...
							<td>
								<s:if test="#info.publishStatus == 1">
									是
			                    </s:if> 
			                    <s:else>
			                    	否
							...
							<td>
							<s:date name="#info.publishTime"	format="yyyy-MM-dd HH:mm:ss" />
							</td>
							<td><s:property value="#info.category.cname" /></td>
							<td>
								<a href='${path}admin/editInfo.action?infoId=<s:property value="#info.infoId"/>'>更新</a> &nbsp;|&nbsp;
								<a href='#'	onclick="del(<s:property value="#info.infoId"/>)">删除</a>
								</td>
						</tr>	
					</s:iterator>
					</c:if>
